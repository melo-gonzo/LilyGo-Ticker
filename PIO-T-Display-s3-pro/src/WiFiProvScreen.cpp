...
void showQRCodeInContainer(const char *service_name, const char *pop) {
  char provQRCode[256];
  snprintf(provQRCode, sizeof(provQRCode),
           "{\"ver\":\"v1\",\"name\":\"%s\",\"pop\":\"%s\",\"transport\":"
           "\"softap\"}",
           service_name, pop);

  lv_obj_t *qrCode = lv_qrcode_create(
      ui_Container_QR, 180, lv_color_hex3(0x000), lv_color_hex3(0xFFF));
  if (qrCode != NULL) {
...
void updateConnectionStatus(const char *status, const char *service_name,
  ...
  snprintf(message, sizeof(message), "%s\nService: %s\nPOP: %s\nTransport: %s",
           status, service_name, pop, transport);
