...
void setupProvisioning(const char *pop, const char *service_name,
                       const char *service_key, bool reset_provisioned);
void updateConnectionStatus(const char *status, const char *service_name,
                            const char *pop, const char *transport);
bool isProvisioned();     // Checks if Wi-Fi credentials are already provisioned
void resetProvisioning(); // Resets Wi-Fi provisioning by erasing stored
                          // credentials
